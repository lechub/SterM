...
public:
  ...
  void co10ms(){
    ...
    if (oldKey != newKey){
      if (((oldKey == Key::ENTER_CANCEL)&&((newKey == Key::ENTER)||(newKey == Key::CANCEL)))  // nie liczyć klawisza przy puszczaniu podwójnych
          ||((oldKey == Key::LEFT_RIGHT)&&((newKey == Key::LEFT)||(newKey == Key::RIGHT)))) {
      }else{
